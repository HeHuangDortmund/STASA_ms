...
singleStimulus = all_stimulus[3]

aggregattionAndLD = function(singleStimulus, OR_p = 0.22 ){
  Fixation_MIT1003_singleStimulus = Fixation_OSIE[Stimulus == singleStimulus]
...
ORs = min_value_theta_osie_gld[min_value_theta_osie_gld$variable == "STASA",]$V1*0.1
all_stimulus = unique(Fixation_OSIE$Stimulus)

res_osie_minGLA_MM_Extra = list()
for (i in 1:length(all_stimulus)) {
  print(paste0('i: ',i))
  
  TMP = try(aggregattionAndLD(all_stimulus[i], ORs[i]))
  if(class(TMP) == 'try-error'){
    res_osie_minGLA_MM_Extra[[i]] = NA
  }else{
    print(paste0('score: ',TMP))
    res_osie_minGLA_MM_Extra[[i]] = TMP
  }
}

save(res_osie_minGLA_MM_Extra, file = file.path(root, 'results_new','res_osie_minGLA_MM_Extra.Rda'))
